...
class Bode(Columns):
    def __init__(self, **kwargs):
        ...
        arrangement = [{'x': 'frequency', 'y': ['magnitude', 'angle']}]

        super().__init__(arrangement, 
                         {'frequency': "Frequency (Hz)"},
                         {'magnitude': "Magnitude (Ω)",
                          'angle': "Phase Angle (°)"},
                         **kwargs
                        )
        
    def _create_figure(self, key_figure):
        if key_figure[1] == 'magnitude':        
            figure = bokeh.plotting.figure(x_axis_type = "log", y_axis_type = "log")
        else:
            figure = bokeh.plotting.figure(x_axis_type = "log")
        return figure
